...
choice_body_type = ('skinny', 'thin', 'fit', 'average', 'athletic', 'jacked', 'curvy', 'a little extra', 'full figured', 'overweight', 'used up', 'rather not say')
...
choice_education = ('college', 'high_school', 'law_school', 'master', 'med_school', 'PhD', 'space_camp')
...
choice_offspring_want = ("doesn't want", "doesn't want more", 'might want', 'might want more', 'wants', 'wants more')
...
choice_sign = ('Aries', 'Taurus', 'Gemini', 'Cancer', 'Leo', 'Virgo', 'Libra', 'Scorpio', 'Sagittarius', 'Capricorn', 'Aquarius', 'Pisces')
...
def offsprings_input():
    ...
    have_kids_option = st.selectbox('Do you have kids?', choice_offspring_have, index=None, placeholder='<select>')
    want_kids_option = st.selectbox('Do you want (more) kids?', choice_offspring_want, index=None, placeholder='<select>')
...
def pet_input():
    ...
    if selected_have:
        if 'no pets' in selected_have and len(selected_have) > 1:
            ...
            have_pets = selected_have[0] if len(selected_have) == 1 else ", ".join(selected_have)
    ...
    if selected_like:
        ...
        if any(conflicts):
            ...
            like_pets = selected_like[0] if len(selected_like) == 1 else ", ".join(selected_like)
...
def reglion_input():
    ...
    religion = st.selectbox('What is your religion? ', choice_religion, index=None, placeholder='<select>')
    attitude = None
    disabled = True
    ...
    if religion == 'Other':
        attitude = st.text_input("Please specify your religion", value=None, max_chars=50)
...
def personal_input():
    ...
    i_dict['age'] = st.number_input(
        "Enter your age:", 
        min_value=limit_age[0], 
        max_value=limit_age[1], 
        step=1, 
        value=None, 
    ...
    i_dict['height'] = st.number_input(
        ...
        min_value=limit_height[0], 
        max_value=limit_height[1], 
        step=1, 
        value=None,
        placeholder="Type a number..."
    )
    ...
    i_dict['ethnicity'] = st.multiselect(
...
def filter_input():
    ...
    filter_button = st.toggle("Show Filters")

    st.session_state.setdefault('filter_dict', {})
    f_dict = st.session_state.filter_dict
    f_dict.setdefault('age_range', (limit_age[0], limit_age[1]))
    f_dict.setdefault('height_range', (limit_height[0], limit_height[1]))
    f_dict.setdefault('location', None)
    f_dict.setdefault('speaks', None)
    f_dict.setdefault('ethnicity', None)
    f_dict.setdefault('religion', None)
    f_dict.setdefault('sign', None)
    f_dict.setdefault('keyword_filter', None)

    if filter_button:
        f_dict['age_range'] = st.slider("Age Range:", min_value=limit_age[0], max_value=limit_age[1], value=f_dict['age_range'])
        f_dict['height_range'] = st.slider("Height Range (cm):", min_value=limit_height[0], max_value=limit_height[1], value=f_dict['height_range'])
        f_dict['location'] = st.text_input("Preferred Location (city, state, or country):", value=f_dict['location'], placeholder="Type a location...")
        f_dict['speaks'] = st.multiselect("Preferred Languages:", options=choice_language, default=f_dict['speaks'])
        f_dict['ethnicity'] = st.multiselect("Preferred Ethnicity:", options=choice_ethnicity, default=f_dict['ethnicity'])
        f_dict['religion'] = st.multiselect("Preferred Religion:", options=choice_religion, default=f_dict['religion'])
        f_dict['sign'] = st.multiselect("Preferred Astrological Sign:", options=choice_sign, default=f_dict['sign'])
        f_dict['keyword_filter'] = st.text_input("Keyword Filter (e.g., hobbies, interests):", value=f_dict['keyword_filter'], placeholder="Type a keyword...")
        clear = st.button('Clear Filters')
        if clear:
            f_dict['age_range'] = (limit_age[0], limit_age[1])
            f_dict['height_range'] = (limit_height[0], limit_height[1])
            f_dict['location'] = None
            f_dict['speaks'] = None
            f_dict['ethnicity'] = None
            f_dict['religion'] = None
            f_dict['sign'] = None
            f_dict['keyword_filter'] = None
...
def filter_dict_split(df):
    ...
    for col in df.columns:
        if '_range' in col:
            ...
            df[[min_col, max_col]] = pd.DataFrame(df[col].tolist(), index=df.index)
    ...
    original_non_range_columns = [col for col in df.columns if col not in new_columns]
...
def select_recommendation():
    st.session_state.top_n = st.slider('Number of Recommendation', min_value=limit_recommendation[0], max_value=limit_recommendation[1], value=limit_recommendation[0], step=5)
...
def recommendation_result(result):
    ...
    if isinstance(st.session_state.recommendations, pd.DataFrame):
        if len(st.session_state.recommendations) == st.session_state.top_n:
            st.success(f"Here are your top {st.session_state.top_n} recommendations:")
            ...
            st.warning(f"Only {len(st.session_state.recommendations)} recommendation(s) found. Please try again with different inputs.")
            st.dataframe(st.session_state.recommendations, width=1000)
    else:
        st.error(f"{st.session_state.recommendations}. No recommendations found. Please try again with different inputs.") 
